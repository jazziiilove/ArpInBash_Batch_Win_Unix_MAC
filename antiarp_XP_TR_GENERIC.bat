...
:GENERIC Static MAC setter batch program for XP TR
...
:@IPCONFIG /ALL | FIND /I "Fiziksel Adres" > %TEMP3%
:@SET LOCALMAC=%%q
...
@SET REALGATEAWAYIP = "139.179.33.1" 
@SET REALGATEAWAYMAC = "00-15-f2-64-a3-38"

:Make gateaway ip and mac static
if "%GATEAWAYIP%" == "%REALGATEAWAYIP%"(
if "%GATEAWAYMAC%" == "%REALGATEAWAYMAC%"(
echo "no problem"
)
)

if not "%GATEAWAYIP%" == "%REALGATEAWAYIP%"(
if not "%GATEAWAYMAC%" == "%REALGATEAWAYMAC%"(
...
ECHO Real Gateaway MAC Address: %REALGATEAWAYMAC%
ECHO Real Gateaway IP Address: %REALGATEAWAYIP%
